...
const PaymentModal: React.FC<PaymentModalProps> = ({ amount, paymentMethod, config, onConfirm, onCancel, redirectUrl, onGatewayInitiate }) => {
    ...
    const handleGatewayPay = async () => {
        ...
        try {
            ...
            console.error(error);
            // [FIX] Suppress "PopupInView" error. This means the popup is ALREADY open, so we don't need to alert the user.
            if (error.message && error.message.includes('PopupInView')) {
                console.warn('Snap Popup is already open. Ignoring re-trigger.');
                return;
            }
